                    ...
                    <div class="task_list" :key="flagKey">
                        <!-- <div class="fixed_tags">
                            ...
                            <el-tag @click.native="changeTask(1)" style="cursor: pointer;"
                                :class="{ 'bgColor': flagNum === 1 }" type="success">韧性提升</el-tag>
                            <el-tag @click.native="changeTask(2)" style="cursor: pointer;"
                                :class="{ 'bgColor': flagNum === 2 }" type="success">可靠性和韧性评估</el-tag>
                            <el-tag @click.native="changeTask(3)" style="cursor: pointer;"
                                :class="{ 'bgColor': flagNum === 3 }" type="success">上传任务</el-tag>
                        </div> -->
                        <div class="card_list">
                ...
                this.timer = setTimeout(() => {
                    ...
                    switch (name) {
                        case "拓扑结构&灾前":
                            ...
                            setTimeout(() => {
                                this.$bus.$emit('FirstTopologyDATA', this.allData[task.num]);
                            }, 50)
                            break;
                        case "拓扑结构&灾后":
                            // this.$parent.toRunMain(this.toolGroups[0][0])
                            setTimeout(() => {
                                this.$bus.$emit('LastTopologyDATA', this.allData[task.num]);
                            }, 50)
                            break;
                        case "拓扑结构&应急发电车规划":
                        case "拓扑结构&应急发电车配置":
                            // this.$parent.toRunMain(this.toolGroups[0][0])
                            setTimeout(() => {
                                this.$bus.$emit('carTopologyDATA', this.allData[task.num]);
                            }, 50)
                            break;
                        case "拓扑结构&awoa":
                        case "拓扑结构&mixture":
                        ...
                        case "拓扑结构&maque":
                            ...
                            setTimeout(() => {
                                this.$bus.$emit('AssessmentDATA', { allData: this.allData[task.num], flag: 1 });
                            }, 50)
                            break;
                        case "拓扑结构&nsga_two":
                            // this.$parent.toRunMain(this.toolGroups[1][0])
                            setTimeout(() => {
                                this.$bus.$emit('AssessmentDATA', { allData: this.allData[task.num], flag: 2 });
                            }, 50)
                            break;
                    }
                    let tipsName = '已提交' + name
                    if (name === '天气数据&风速预测模型(训练)' || name === '故障模拟数据&故障诊断(训练)' || name === '故障模拟数据&故障定位-基于attention-lstm的故障定位(训练)') {
                ...
                .then(response => {
                    ...
                    if (response && response.code === 200) {
                        ...
                        if (response.rows) {
                            ...
                            this.tasks = [...this.task1, ...this.task2, ...this.uploadTasks]
