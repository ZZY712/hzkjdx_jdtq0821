                    ...
                    <div class="task_list" :key="flagKey">
                        <!-- <div class="fixed_tags">
                            ...
                            <el-tag @click.native="changeTask(1)" style="cursor: pointer;"
                                :class="{ 'bgColor': flagNum === 1 }" type="success">极端天气</el-tag>
                            <el-tag @click.native="changeTask(7)" style="cursor: pointer;"
                                :class="{ 'bgColor': flagNum === 7 }" type="success">覆冰-冰冻</el-tag>
                            <el-tag @click.native="changeTask(2)" style="cursor: pointer;"
                                :class="{ 'bgColor': flagNum === 2 }" type="success">风险评估</el-tag>
                            <el-tag @click.native="changeTask(3)" style="cursor: pointer;"
                                :class="{ 'bgColor': flagNum === 3 }" type="success">风险预警</el-tag>
                            <el-tag @click.native="changeTask(4)" style="cursor: pointer;"
                                :class="{ 'bgColor': flagNum === 4 }" type="success">故障诊断</el-tag>
                            <el-tag @click.native="changeTask(5)" style="cursor: pointer;"
                                :class="{ 'bgColor': flagNum === 5 }" type="success">小样本增广</el-tag>
                            <el-tag @click.native="changeTask(6)" style="cursor: pointer;"
                                :class="{ 'bgColor': flagNum === 6 }" type="success">上传任务</el-tag>
                        </div> -->
                        <div class="card_list">
                            <el-card v-for="(task, index) in tasks" :key="index" class="task_card">
                                ...
                                <el-tag v-if="task.completed === 'T'" type="warning" style="cursor: pointer;"
                                    @click.native="successRun(task)">等待运行</el-tag>
                                ...
                                <el-tag v-if="task.completedInput === 'T'" type="warning"
                                    style="cursor: pointer;">等待上传</el-tag>
            ...
            if (task.completed === 'S') {
                ...
                this.timer = setTimeout(() => {
                    ...
                    switch (name) {
                        case '天气数据&风速预测模型(预测)':
                        case '天气数据&天气预测模型(预测)':
                        case '天气数据&新能源预测模型(训练)':
                        case "拓扑结构&覆冰预测模型(预测)":
                        case '拓扑结构&极端天气灾害下的配电网停电风险智能预警':
                        case '拓扑结构&雷击环境下故障概率':
                            // this.$parent.toRunMain(this.toolGroups[0][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataWind', this.allData[task.num]);
                            }, 50)
                            break;
                        case '天气数据&ARG模型(训练)':
                        case '天气数据&故障预测模型(训练)':
                            ...
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataARG', { allData: this.allData[task.num], flag: 1 });
                            }, 50)
                            break;
                        case '天气数据&ARG模型(预测)':
                        case '天气数据&故障预测模型(预测)':
                            ...
                            // this.$parent.toRunMain(this.toolGroups[1][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataARG', { allData: this.allData[task.num], flag: 2 });
                            }, 50)
                            break;
                        case '拓扑结构&台风模型线路故障概率':
                            // 发送请求数据
                            // this.$parent.toRunMain(this.toolGroups[1][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataARG', { allData: this.allData[task.num], flag: 3 });
                            }, 50)
                            break;
                        case "天气数据&故障预测数学模型":
                            // 发送请求数据
                            // this.$parent.toRunMain(this.toolGroups[1][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataARG', { allData: this.allData[task.num], flag: 4 });
                            }, 50)
                            break;
                        case '天气数据&基于电流的风险预警(训练)':
                            // this.$parent.toRunMain(this.toolGroups[2][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataGZ', { allData: this.allData[task.num], flag: 1 });
                            }, 50)
                            break;
                        case '天气数据&基于电流的风险预警(预测)':
                            // this.$parent.toRunMain(this.toolGroups[2][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataGZ', { allData: this.allData[task.num], flag: 2 });
                            }, 50)
                            break;
                        case '天气数据&基于电流的风险预警(pytorch训练)':
                            // this.$parent.toRunMain(this.toolGroups[2][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataGZ', { allData: this.allData[task.num], flag: 3 });
                            }, 50)
                            break;
                        case '天气数据&基于电流的风险预警(pytorch预测)':
                            // this.$parent.toRunMain(this.toolGroups[2][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataGZ', { allData: this.allData[task.num], flag: 4 });
                            }, 50)
                            break;
                        case '故障模拟数据&故障诊断(训练)':
                            // this.$parent.toRunMain(this.toolGroups[3][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataZD', { allData: this.allData[task.num], flag: 1 });
                            }, 50)
                            break;
                        case '故障模拟数据&故障诊断(预测)':
                            // this.$parent.toRunMain(this.toolGroups[3][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataZD', { allData: this.allData[task.num], flag: 2 });
                            }, 50)
                            break;
                        case '配网结构数据&故障定位-容错性测试':
                        case '拓扑结构&故障定位-基于ftu信息的故障定位':
                        case '配网结构数据&故障定位-大规模配电网测试':
                        case '配网结构数据&故障定位-性能比较':
                        case '配网结构数据&故障定位-性能比较(容错)':
                        case '拓扑结构&故障定位-故障分类':
                        case '拓扑结构&故障定位-容错性测试':
                        case '拓扑结构&故障定位-极端天气灾害下基于FTU信号丢失的有源配电网故障区段定位方法':
                            // this.$parent.toRunMain(this.toolGroups[3][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataZD', { allData: this.allData[task.num], flag: 3 });
                            }, 50)
                            break;
                        case '故障模拟数据&故障定位-基于attention-lstm的故障定位(训练)':
                        case '故障模拟数据&故障定位-基于attention-lstm的故障定位(预测)':
                            // this.$parent.toRunMain(this.toolGroups[3][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataZD', { allData: this.allData[task.num], flag: 4 });
                            }, 50)
                            break;
                        case '天气数据&GAN模型(训练)':
                            // 发送请求数据
                            // this.$parent.toRunMain(this.toolGroups[4][0])
                            setTimeout(() => {
                                this.$bus.$emit('runQueueDataGAN', this.allData[task.num]);
                            }, 50)
                            break;
                        default:
                            break;
                    ...
                    if (name === '天气数据&天气预测模型(训练)' || name === '故障模拟数据&故障诊断(训练)' || name === '故障模拟数据&故障定位-基于attention-lstm的故障定位(训练)'
                        || name === '天气数据&覆冰预测模型(训练)'
                    ) {
                        this.$notify({ title: tipsName, message: '该数据不进行渲染', type: 'warning' });
                    } else {
                                ...
                                if (item.status != 'O' && item.status != 'WX') {
                                    ...
                                    if (item.data_id != "D000000") {
                                        ...
                                        this.uploadTasks.push({
                                            ...
                                            num: index,
                                            data_id: item.data_id,
                                            data_type: item.data_type, task_id: item.task_id,
                                        })
                                    }
                                }
                            })
                            this.tasks = [...this.task1, ...this.task2, ...this.task3, ...this.task4, ...this.task5, ...this.uploadTasks]
